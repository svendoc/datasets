...
def load_gap_splits(use_validation):
    return load_modified_gap_splits(use_validation, 1.0 / 3.0)
...
def load_modified_gap_splits(use_validation, test_size):
    ...
    for dataset, (x, y) in load_datasets().items():
        ...
        assert (2 * tail_size + test_size) == 1.0
        ...
        for dim in range(m):
            ...
            if use_validation:
                tr, val = train_test_split(tr, test_size=0.1)
                val = val.astype(int)
            else:
                val = None
        ...
        if dataset.is_dir() and (dataset / "data").exists():
            ...
            for i in range(n):
                ...
                if use_validation:
                    ...
                    assert (
                        val.dtype == int
                    ), "UCI Validations indices must have int type"
                else:
                    val = None
                te = loadtxt(
                    dataset / "data" / f"{'_'.join(('index', 'test', str(i)))}.txt"
                ).astype(int)
...
def pprint_summary_latex(split, use_validation):
    ...
    match split:
        ...
        case "gap10":
            indices = load_modified_gap_splits(use_validation, 1.0 / 10.0)
